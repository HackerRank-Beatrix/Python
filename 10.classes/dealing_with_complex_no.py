...
class Complex(object):
    ...
    def __truediv__(self, no):
        factor = 1 / (no.real * no.real + no.imaginary * no.imaginary)
        x = factor * (self.real * no.real + self.imaginary * no.imaginary)
        y = factor * (self.imaginary * no.real - self.real * no.imaginary)
        return Complex(x, y)
